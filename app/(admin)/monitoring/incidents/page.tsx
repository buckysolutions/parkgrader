...
function IncidentsPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch(`/api/admin/monitoring/incidents?status=${filter}&admin_key=${adminKey}`);
    ...
    <div className="space-y-8">
          ...
          {(["open", "resolved"] as const).map((f) => (
            <Link
              ...
              className={`rounded-lg px-3 py-1.5 text-sm font-medium capitalize transition ${
                filter === f ? "bg-white text-[#0A1628] shadow-sm" : "text-[#5B6776] hover:text-[#0A1628]"
              }`}
            >
              {f}
            </Link>
      ...
      {loading ? (
        ...
        <div className="glass-card rounded-2xl bg-white py-16 text-center">
          <p className="text-[#8C97A8]">No {filter} incidents</p>
        </div>
      ) : (
        <div className="space-y-3">
          {incidents.map((i) => (
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <div className="flex items-center gap-2">
                    <Link
                      href={`/monitoring/websites/${i.id}${keyParam}`}
                      className="font-medium text-[#0A1628] hover:text-[#2DA4A9]"
                    >
                      {i.website.businessName}
                    </Link>
                    <span className="text-sm text-[#8C97A8]">{i.website.domain}</span>
                  </div>
