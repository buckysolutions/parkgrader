...
function NotificationsPage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        const res = await fetch(`/api/admin/monitoring/notifications?status=pending&admin_key=${adminKey}`);
    ...
    <div className="space-y-8">
      ...
      {loading ? (
        <div className="space-y-3">
          {[...Array(2)].map((_, i) => <div key={i} className="h-28 animate-pulse rounded-2xl bg-gray-100" />)}
        </div>
              ...
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-sm capitalize text-[#5B6776]">
                    {n.type.replace(/_/g, " ")}
                    {n.email && <span className="text-[#8C97A8]"> — {n.email}</span>}
                  </p>
