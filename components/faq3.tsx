    ...
    <section className={cn("py-32", className)}>
      <div className="mx-auto max-w-3xl space-y-8 px-6">
        <div className="mx-auto flex max-w-3xl flex-col text-center">
          <h2 className="mb-3 text-[1.75rem] font-semibold text-[#0A1628] sm:text-[2.4rem] md:mb-4 lg:mb-6">
            {heading}
          </h2>
          <p className="text-lg text-[#5B6776]">{description}</p>
        </div>
        <Accordion
          ...
          {items.map((item) => (
            <AccordionItem key={item.id}>
              <AccordionTrigger className="transition-opacity duration-200 hover:no-underline hover:opacity-60">
                <div className="font-medium text-lg text-[#0A1628] sm:py-1 lg:py-2">
                  {item.question}
                </div>
              </AccordionTrigger>
              <AccordionContent className="sm:mb-1 lg:mb-2">
                <div className="text-base text-[#5B6776] lg:text-lg">
                  {item.answer}
                </div>
              </AccordionContent>
            </AccordionItem>
